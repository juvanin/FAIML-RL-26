...
- [What is Domain Randomization?](https://lilianweng.github.io/posts/2019-05-05-domain-randomization/#what-is-domain-randomization)
- [Uniform Domain Randomization](https://lilianweng.github.io/posts/2019-05-05-domain-randomization/#uniform-domain-randomization)
- [Why does Domain Randomization Work?](https://lilianweng.github.io/posts/2019-05-05-domain-randomization/#why-does-domain-randomization-work)  - [DR as Optimization](https://lilianweng.github.io/posts/2019-05-05-domain-randomization/#dr-as-optimization)
  - [DR as Meta-Learning](https://lilianweng.github.io/posts/2019-05-05-domain-randomization/#dr-as-meta-learning)
- [Guided Domain Randomization](https://lilianweng.github.io/posts/2019-05-05-domain-randomization/#guided-domain-randomization)  - [Optimization for Task Performance](https://lilianweng.github.io/posts/2019-05-05-domain-randomization/#optimization-for-task-performance)
  - [Match Real Data Distribution](https://lilianweng.github.io/posts/2019-05-05-domain-randomization/#match-real-data-distribution)
  - [Guided by Data in Simulator](https://lilianweng.github.io/posts/2019-05-05-domain-randomization/#guided-by-data-in-simulator)
- [References](https://lilianweng.github.io/posts/2019-05-05-domain-randomization/#references)
...
![](https://lilianweng.github.io/posts/2019-05-05-domain-randomization/sim2real-transfer.png)Conceptual illustrations of three approaches for sim2real transfer.

# What is Domain Randomization? [\#](https://lilianweng.github.io/posts/2019-05-05-domain-randomization/\#what-is-domain-randomization)

To make the definition more general, let us call the environment that we have full access to (i.e. simulator) **source domain** and the environment that we would like to transfer the model to **target domain** (i.e. physical world). Training happens in the source domain. We can control a set of N randomization parameters in the source domain eξ with a configuration ξ, sampled from a randomization space, ξ∈Ξ⊂RN.

During policy training, episodes are collected from source domain with randomization applied. Thus the policy is exposed to a variety of environments and learns to generalize. The policy parameter θ is trained to maximize the expected reward R(.) average across a distribution of configurations:

θ∗=arg⁡maxθEξ∼Ξ\[Eπθ,τ∼eξ\[R(τ)\]\]

where τξ is a trajectory collected in source domain randomized with ξ. In a way, _“discrepancies between the source and target domains are modeled as variability in the source domain.”_ (quote from [Peng et al. 2018](https://arxiv.org/abs/1710.06537)).

# Uniform Domain Randomization [\#](https://lilianweng.github.io/posts/2019-05-05-domain-randomization/\#uniform-domain-randomization)

In the original form of DR ( [Tobin et al, 2017](https://arxiv.org/abs/1703.06907); [Sadeghi et al. 2016](https://arxiv.org/pdf/1611.04201.pdf)), each randomization parameter ξi is bounded by an interval, ξi∈\[ξilow,ξihigh\],i=1,…,N and each parameter is uniformly sampled within the range.
...
![](https://lilianweng.github.io/posts/2019-05-05-domain-randomization/DR.png)Images captured in the training environment are randomized. (Image source: [Tobin et al, 2017](https://arxiv.org/abs/1703.06907))
...
[Learning Dexterity: Uncut](https://www.youtube.com/watch?v=DKe8FumoD4E) [OpenAI](https://www.youtube.com/channel/UCXZCJLdBC09xxGZ6gcdrc6A)

OpenAI1.95M subscribers

[Watch on](https://www.youtube.com/watch?v=DKe8FumoD4E)

# Why does Domain Randomization Work? [\#](https://lilianweng.github.io/posts/2019-05-05-domain-randomization/\#why-does-domain-randomization-work)
...
## DR as Optimization [\#](https://lilianweng.github.io/posts/2019-05-05-domain-randomization/\#dr-as-optimization)

One idea ( [Vuong, et al, 2019](https://arxiv.org/abs/1903.11774)) is to view learning randomization parameters in DR as a _bilevel optimization_. Assuming we have access to the real environment ereal and the randomization config is sampled from a distribution parameterized by ϕ, ξ∼Pϕ(ξ), we would like to learn a distribution on which a policy πθ is trained on can achieve maximal performance in ereal:

ϕ∗=arg⁡minϕL(πθ∗(ϕ);ereal)where θ∗(ϕ)=arg⁡minθEξ∼Pϕ(ξ)\[L(πθ;eξ)\]

where L(π;e) is the loss function of policy π evaluated in the environment e.

Although randomization ranges are hand-picked in uniform DR, it often involves domain knowledge and a couple rounds of trial-and-error adjustment based on the transfer performance. Essentially this is a manual optimization process on tuning ϕ for the optimal L(πθ∗(ϕ);ereal).
...
# Guided Domain Randomization [\#](https://lilianweng.github.io/posts/2019-05-05-domain-randomization/\#guided-domain-randomization)
...
## Optimization for Task Performance [\#](https://lilianweng.github.io/posts/2019-05-05-domain-randomization/\#optimization-for-task-performance)

Say we train a family of policies with different randomization parameters ξ∼Pϕ(ξ), where Pξ is the distribution for ξ parameterized by ϕ. Later we decide to try every one of them on the downstream task in the target domain (i.e. control a robot in reality or evaluate on a validation set) to collect feedback. This feedback tells us how good a configuration ξ is and provides signals for optimizing ϕ.
...
[Ruiz (2019)](https://arxiv.org/abs/1810.02513) considered the _task feedback_ as _reward_ in RL problem and proposed a RL-based method, named “learning to simulate”, for adjusting ξ. A policy is trained to predict ξ using performance metrics on the validation data of the main task as rewards, which is modeled as a multivariate Gaussian. Overall the idea is similar to AutoAugment, applying NAS on data generation. According to their experiments, even if the main task model is not converged, it still can provide a reasonable signal to the data generation policy.

![](https://lilianweng.github.io/posts/2019-05-05-domain-randomization/learning-to-simulate.png)An overview of the "learning to simulate" approach. (Image source: [Ruiz (2019)](https://arxiv.org/abs/1810.02513))

Evolutionary algorithm is another way to go, where the _feedback_ is treated as _fitness_ for guiding evolution ( [Yu et al, 2019](https://openreview.net/forum?id=H1g6osRcFQ)). In this study, they used [CMA-ES](https://en.wikipedia.org/wiki/CMA-ES) (covariance matrix adaptation evolution strategy) while fitness is the performance of a ξ-conditional policy in target environment. In the appendix, they compared CMA-ES with other ways of modeling the dynamics of ξ, including Bayesian optimization or a neural network. The main claim was those methods are not as stable or sample efficient as CMA-ES. Interestly, when modeling P(ξ) as a neural network, LSTM is found to notably outperform FF.
...
Using real data to guide domain randomization feels a lot like doing system identification or DA. The core idea behind DA is to improve the synthetic data to match the real data distribution. In the case of real-data-guided DR, we would like to learn the randomization parameters ξ that bring the state distribution in simulator close to the state distribution in the real world.

The **SimOpt** model ( [Chebotar et al, 2019](https://arxiv.org/abs/1810.05687)) is trained under an initial randomization distribution Pϕ(ξ) first, getting a policy πθ,Pϕ. Then this policy is deployed on both simulator and physical robot to collect trajectories τξ and τreal respectively. The optimization objective is to minimize the discrepancy between sim and real trajectories:

ϕ∗=arg⁡minϕEξ∼Pϕ(ξ)\[Eπθ,Pϕ\[D(τsim,τreal)\]\]
...
![](https://lilianweng.github.io/posts/2019-05-05-domain-randomization/simopt.png)An overview of the SimOpt framework. (Image source: [Chebotar et al, 2019](https://arxiv.org/abs/1810.05687))
...
![](https://lilianweng.github.io/posts/2019-05-05-domain-randomization/RCAN.png)RCAN is an image-conditional generator that can convert a domain-randomized or real image into its corresponding non-randomized simulator version. (Image source: [James et al., 2019](https://arxiv.org/abs/1812.07252))
...
G: randomized image → {canonical image, segmentation, depth}
...
![](https://lilianweng.github.io/posts/2019-05-05-domain-randomization/deception-net.png)How DeceptionNet works. (Image source: [Zakharov et al., 2019](https://arxiv.org/abs/1904.02750))
...
![](https://lilianweng.github.io/posts/2019-05-05-domain-randomization/ADR.png)How active domain randomization (ADR) works. (Image source: [Mehta et al., 2019](https://arxiv.org/abs/1904.04762))
...
[Go to Top (Alt + G)](https://lilianweng.github.io/posts/2019-05-05-domain-randomization/#top "Go to Top (Alt + G)")